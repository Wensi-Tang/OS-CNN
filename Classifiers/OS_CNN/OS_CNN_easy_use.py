...
class OS_CNN_easy_use():
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        train_loader = DataLoader(train_dataset, batch_size=int(min(X_train.shape[0] / 10, self.batch_size)), shuffle=True)
        test_dataset = TensorDataset(X_test, y_test)
        test_loader = DataLoader(test_dataset, batch_size=int(min(X_train.shape[0] / 10, self.batch_size)), shuffle=False)
